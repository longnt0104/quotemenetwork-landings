...
<head>
  ...
  <title>Get Free Painting Quotes | QuoteMeNetwork</title>
  <meta name="description" content="Request free painting quotes from local pros." />
  <style>
    ...
    .card { background: var(--card); border: 1px solid var(--border); border-radius: 14px; box-shadow: 0 10px 24px rgba(0,0,0,0.06); padding: 26px; }
    .top { text-align: center; margin-bottom: 18px; }
    .top h1 { font-size: 26px; margin: 0 0 8px; color: var(--brand); }
    .top p { margin: 0; color: var(--muted); line-height: 1.35; }

    .progress { display:flex; gap:8px; margin: 18px 0 22px; }
    .bar { height: 8px; border-radius: 999px; background: #eef2ff; flex: 1; overflow: hidden; border: 1px solid var(--border); }
    .bar > div { height: 100%; background: var(--brand); width: 0%; transition: width .25s ease; }
    ...
    .opt { width: 100%; padding: 18px 14px; border-radius: 12px; border: 1px solid var(--border); background: #ffffff; cursor: pointer; font-size: 18px; transition: transform .04s ease, border-color .12s ease, box-shadow .12s ease; text-align: left; }
    .opt:hover { border-color: rgba(0,0,0,0.06); box-shadow: 0 10px 20px rgba(0,0,0,0.06); }
    .opt:active { transform: scale(0.99); }
    .opt strong { display:block; font-size: 18px; color: var(--brand); }
    .opt span { display:block; margin-top: 6px; color: var(--muted); font-size: 14px; }
    ...
    .field input { width: 100%; padding: 14px 12px; border-radius: 10px; border: 1px solid var(--border); font-size: 16px; outline: none; background: #fff; }
    .field input:focus { border-color: rgba(0,0,0,0.18); box-shadow: 0 0 0 4px rgba(0,0,0,0.06); }

    .navBtns { display:flex; gap: 10px; justify-content: space-between; margin-top: 18px; flex-wrap: wrap; }
    .btn { border: none; border-radius: 10px; padding: 14px 18px; font-size: 16px; cursor: pointer; }
    ...
    .fineprint { margin-top: 14px; font-size: 12px; color: var(--muted); line-height: 1.35; text-align:center; }
    .fineprint a { color: var(--brand); text-decoration: none; }
    .fineprint a:hover { text-decoration: underline; }

    .error { margin-top: 12px; background: #fff5f5; border: 1px solid #ffd1d1; color: #7f1d1d; border-radius: 10px; padding: 12px; display:none; text-align:left; }
  </style>
</head>
<body>
...
<div class="wrap">
  <div class="card">
    <div class="top">
      <h1>Get Free Painting Quotes</h1>
    ...
    <div class="progress" aria-hidden="true">
      ...
      <div class="bar"><div id="p5"></div></div>
    </div>
...
<input type="hidden" name="_service" value="painting">
      ...
      <section class="step active">
        ...
        <div class="options">
          <button type="button" class="opt" data-set="project_type" data-val="Interior Painting">
            <strong>Interior Painting</strong>
            <span>Walls/ceilings/trim</span>
          </button>
          <button type="button" class="opt" data-set="project_type" data-val="Exterior Painting">
            <strong>Exterior Painting</strong>
            <span>Siding/trim/exterior</span>
          </button>
          <button type="button" class="opt" data-set="project_type" data-val="Cabinet Painting">
            <strong>Cabinet Painting</strong>
            <span>Kitchen/bath cabinets</span>
          </button>
          <button type="button" class="opt" data-set="project_type" data-val="Touch-ups">
            <strong>Touch-ups</strong>
      ...
      <section class="step">
        <h2>Project size</h2>
        <div class="options">
          <button type="button" class="opt" data-set="project_size" data-val="1–2 rooms">
            <strong>1–2 rooms</strong>
            <span>Small</span>
          </button>
          <button type="button" class="opt" data-set="project_size" data-val="3–5 rooms">
            <strong>3–5 rooms</strong>
            <span>Medium</span>
          </button>
          <button type="button" class="opt" data-set="project_size" data-val="Whole home">
            <strong>Whole home</strong>
            <span>Large</span>
          </button>
          <button type="button" class="opt" data-set="project_size" data-val="Not sure">
            <strong>Not sure</strong>
            <span>Estimate needed</span>
          </button>
        </div>
        <input type="hidden" name="project_size" id="project_size" required>
      </section>
      <section class="step">
        <h2>Surface condition</h2>
        <div class="options">
          <button type="button" class="opt" data-set="condition" data-val="Good">
            <strong>Good</strong>
            <span>Minimal prep</span>
          </button>
          <button type="button" class="opt" data-set="condition" data-val="Some repairs">
            <strong>Some repairs</strong>
            <span>Patch/caulk needed</span>
          </button>
          <button type="button" class="opt" data-set="condition" data-val="Major prep">
            <strong>Major prep</strong>
            <span>Peeling/cracks</span>
          </button>
        </div>
...
<script>
  ...
  const hiddenIds = ["project_type", "project_size", "condition", "timeline"];
